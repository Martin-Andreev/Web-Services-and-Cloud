namespace Battleships.WebServices.Controllers
{
    ...
    public class GamesController : BaseApiController
    {
        ...
        public IHttpActionResult PlayTurn(PlayTurnBindingModel model)
        {
            ...
            {
                return this.BadRequest("Second player needed to start");
            }
        ...
        [HttpPost]
        [ActionName("status")]
        public IHttpActionResult GameStatus(GameStatusBindingModel model)
        {
            if (model == null)
            {
                this.ModelState.AddModelError("model", "The model is empty");
            ...
            var game = this.Data.Games
                .All()
                .FirstOrDefault(g => g.Id == guidGameId);
            if (game == null)
            {
                return this.NotFound();
            }
            ...
            if (game.PlayerOneId != userId && game.PlayerTwoId != userId)
            {
                return this.BadRequest("You can't check the status of this game!");
            }
